...
namespace MyNet.CCTV.Control.Implement.DaHua
{
    ...
    public struct DEV_EVENT_TRAFFIC_PARKINGSPACEPARKING_INFO 
    {
         ...
         [MarshalAs(UnmanagedType.ByValArray, SizeConst = 128)]
         public  char[]   szName;      // 事件名称
	     [MarshalAs(UnmanagedType.ByValArray, SizeConst = 8)]
         public  char[]   bReserved1;                  // 字节对齐
	     public UInt32	  PTS;							// 时间戳(单位是毫秒)
	    ...
	    [MarshalAs(UnmanagedType.ByValArray, SizeConst = 3)] 
         public byte[]  byReserved;
	     public UInt32  dwSnapFlagMask;	                // 抓图标志(按位)，具体见NET_RESERVED_COMMON	
    ...
    ///</summary>
    public struct DEV_EVENT_TRAFFIC_MANUALSNAP_INFO
    {
         /// <summary>
        /// 通道号
        /// </summary>
    ...
    public struct DEV_ENCODER_INFO
    {
        ...
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 16)]
        public byte[] szDevIp;			// 前端DVR的IP地址
        public UInt16 wDevPort;							// 前端DVR的端口号
        public byte bDevChnEnable;                      // 解码通道使能
        public byte byDecoderID;						// 对应解码通道号
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 16)]
        public byte[] szDevUser;	// 用户名
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 16)]
        public byte[] szDevPwd;	// 密码
        public Int32 nDevChannel;						// 通道号
        public Int32 nStreamType;						// 码流类型，0：主码流；1：子码流; 2:snap
        public byte byConnType;							// 0：TCP；1：UDP；2：组播
        public byte byWorkMode;							// 0：直连；1：转发
        public UInt16 wListenPort;						// 指示侦听服务的端口，转发时有效
        public UInt32 dwProtoType;						// 协议类型,
        // 0:兼容以前
        // 1:大华二代协议
        // 2:大华系统集成协议
        // 3:大华DSS协议
        // 4:大华rtsp协议
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 64)]
        public byte[] szDevName;						// 前端设备名称
        public byte byVideoInType;                      // 该解码通道当前视频源类型:0-标清(SD),1-高清(HD),注:设备支持高清通道才有效	
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 64)]
        public byte[] szDevIpEx;// szDevIp扩展，前端DVR的IP地址(可以输入域名)
        ...
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 3)]
        public byte[] bReserved;                       // 保留字节
        public UInt32 dwHttpPort;                         // Http端口号,0-65535
        public UInt32 dwRtspPort;                         // Rtsp端口号,0-65535
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 32)]
        public byte[] szChnName;						// 远程通道名称, 只有读取到的名称不为空时才可以修改该通道的名称
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 4)]
        public byte[] reserved;
