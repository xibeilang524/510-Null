...
namespace MyNet.CCTV.Control.Implement.DaHua
{
    /// <summary>
...
namespace MyNet.CCTV.Control.Implement.DaHua
{
    ...
    public sealed class DHClient
    {
        ...
        public static bool DHInit(fDisConnect cbDisConnect, IntPtr dwUser)
        {
            ...
            if (initialized == false)
            {
                //注意：CLIENT_Init不能多次初始化，多次初始会导致电脑突然死机，2014-11-17 by Jan 备注
        ...
        public static void DHSetNetWorkParam(NET_PARAM netpara)
        {
            IntPtr ptr = Marshal.AllocHGlobal(Marshal.SizeOf(netpara));
            Marshal.StructureToPtr(netpara, ptr, true);
            CLIENT_SetNetworkParam(ptr);
        ...
        /// 设置离线重连
        /// </summary>
        /// <param name="cAutoConnect"></param>
        /// <param name="dwUser"></param>
        /// <returns></returns>
        public static bool DHSetAutoReconnect(fHaveReConnect cAutoConnect, IntPtr dwUser)
        {
            bool returnValue = false;
            if (initialized == true)
            {
                returnValue = CLIENT_SetAutoReconnect(cAutoConnect, dwUser);
                DHThrowLastError(returnValue);
            }
            return returnValue;
        }
        ...
        public static bool DHPTZControl(int lLoginID, int nChannelID, PTZ_CONTROL dwPTZCommand, ushort dwStep, bool dwStop)
        {
            ...
            switch (dwPTZCommand)
            {
                ...
                case PTZ_CONTROL.PTZ_ZOOM_DEC_CONTROL:
                    returnValue = CLIENT_DHPTZControl(lLoginID, nChannelID, (ushort)dwPTZCommand, 0 , dwStep, 0 , dwStop);
                    break;
                case PTZ_CONTROL.PTZ_LAMP_CONTROL:
        ...
        public static int DHSwitchDecTVEncoder(int lLoginID, int nDecoderID, ref DEV_ENCODER_INFO lpEncoderInfo, IntPtr dwUser)
        {
            return CLIENT_SwitchDecTVEncoder(lLoginID, nDecoderID, ref lpEncoderInfo, dwUser);
        }

        public static bool DHQueryDecoderInfo(int lLoginID, out DEV_DECODER_INFO lpDecoderInfo, int timeout)
        {
            return CLIENT_QueryDecoderInfo(lLoginID, out lpDecoderInfo, timeout);
        }

        public static bool DHCtrlDecTVScreen(int lLoginID, int nMonitorID, bool bEnable, int nSplitType, byte[] pEncoderChannel, int nBufLen, IntPtr dwUser)
        {
            return CLIENT_CtrlDecTVScreen(lLoginID, nMonitorID, bEnable, nSplitType, pEncoderChannel, nBufLen, dwUser);
        }
        ...
        [DllImport("dhnetsdk.dll")]
        private static extern void CLIENT_SetNetworkParam(IntPtr param);
        ...
        /// <returns></returns>
        [DllImport("dhnetsdk.dll")]
        private static extern bool CLIENT_SetAutoReconnect(fHaveReConnect cbAutoConnect, IntPtr dwUser);
        ...
        [DllImport("dhnetsdk.dll")]
        private static extern int CLIENT_SwitchDecTVEncoder(int lLoginID, int nDecoderID, ref DEV_ENCODER_INFO lpEncoderInfo, IntPtr dwUser);


        [DllImport("dhnetsdk.dll")]
        private static extern bool CLIENT_QueryDecoderInfo(int lLoginID, out DEV_DECODER_INFO lpDecoderInfo, int timeout);

        [DllImport("dhnetsdk.dll")]
        private static extern bool CLIENT_CtrlDecTVScreen(int lLoginID, int nMonitorID, bool bEnable, int nSplitType, byte[] pEncoderChannel, int nBufLen, IntPtr dwUser);
    ...
    public sealed class EventIvs
    {
        ...
        /// 车位有车事件
        /// </summary>
        public static UInt32 EVENT_IVS_TRAFFIC_PARKINGSPACEPARKING = 0x0000012B;	// 车位有车事件

        /// <summary>
        /// 车位无车事件
        /// </summary>
        public static UInt32 EVENT_IVS_TRAFFIC_PARKINGSPACENOPARKING = 0x0000012C;	// 车位无车事件	
    }
    #endregion
    ...
    /// <param name="dwUser"></param>
    public delegate void fHaveReConnect(
        int lLoginID, StringBuilder pchDVRIP, int nDVRPort, IntPtr dwUser);
