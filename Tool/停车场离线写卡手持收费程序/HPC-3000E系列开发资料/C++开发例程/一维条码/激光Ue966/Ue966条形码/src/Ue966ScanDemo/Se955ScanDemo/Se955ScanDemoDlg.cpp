...
void CSe955ScanDemoDlg::OnBnClickedStart()
{
	 ...
	 *  获取串口号启动通信
	 */	
	if(Ue966SerialInit(GucComNo,ucBaud,STOP_BITONE,DAT_NONE) != INIT_OK) {
...
{
	// TODO: Add your control notification handler code here
	if (Ue966DecodeState(TRUE) != SUCCESS_SETTING) {
...
{
	// TODO: Add your control notification handler code here
	if (Ue966DecodeState(FALSE) != SUCCESS_SETTING) {
		AfxMessageBox(_T("设置错误！"));
	}
}
...
void CSe955ScanDemoDlg::OnBnClickedSofttrig()
{
	...
	if (Ue966TriggerState(SCAN_HOST) != SUCCESS_SETTING) {
		AfxMessageBox(_T("设置错误！"));
	}
...
void CSe955ScanDemoDlg::OnBnClickedContinuetrig()
{
	...
	if (Ue966TriggerState(SCAN_CONTINUE) != SUCCESS_SETTING) {
		AfxMessageBox(_T("设置错误！"));
	}
...
DWORD CSe955ScanDemoDlg::ThreadProc(PVOID pArg)
{
	...
	while(TRUE) {
		 ...
		 *  判断接收数据是否成功
		 */
		ulState = Ue966GetDecodeData(ReceiveBuf, &ucCount);
