...
namespace Ralid.Parking.POS.Tool
{
    ...
    public  class SEBinaryConverter
    {
        ...
        /// <param name="t"></param>
        /// <returns>返回值为一个长度为8的字节数组</returns>
        public static byte[] ULongToBytes(ulong t)
        {
            byte[] bytes = new byte[8];
        ...
        /// <param name="t"></param>
        /// <returns>返回值是一个长度为2的字节数组</returns>
        public static byte[] UShortToBytes(ushort t)
        {
            byte[] bytes = new byte[2];
        ...
        public static ulong BytesToULong(byte[] bytes)
        {
            ...
            if (bytes.Length <= 8)
            {
                for (int i = 0; i < bytes.Length; i++)
                {
                    temp += (ulong)bytes[i] << (i * 8);
                }
                return temp;
            }
            else
            {
                throw new InvalidCastException("转换成无符号长整形数最多只能是8个字节");
            }
        }
        ...
        public static ushort BytesToUShort(byte[] bytes)
        {
            ...
            {
                throw new InvalidCastException("转换成无符号短整数最多只能是两个字节");
            }
        }
        #endregion
    }
